...
class DelayService
{
    ...
    public function show($request)
    {
        ...
        if (!isset($queryData['is_process'])) {
            $query->where('is_process', '=', true);
        }
        if (isset($queryData['orderBy'])) {
            $query->orderBy('approximate_time', $queryData['orderBy']);
            unset($queryData['orderBy']);
        }
        $query->where($queryData);

        return DelayResource::collection($query->get());
    }

    public function getInfoForAnalyst($request)
    {
        $queryData = $request->input();
